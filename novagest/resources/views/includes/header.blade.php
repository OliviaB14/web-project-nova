<ul id="dropdown0" class="dropdown-content">
  <!-- Toutes les pages -->
  <li><a href="agences">Agences</a></li>
  <li><a href="clients">Clients</a></li>
  <li><a href="droits">Droits</a></li>
  <li><a href="droittypeutilisateurs">droittypeutilisateur</a></li>
  <li><a href="historiquevehicules">historiquevehicule</a></li>
  <li><a href="piecevehicules">piecevehicule</a></li>
  <li><a href="singleVehicules">singleVehicule</a></li>
  <li><a href="statutvehicules">statutvehicule</a></li>
  <li><a href="typeclients">typeclient</a></li>
  <li><a href="typeetatpieces">typeetatpiece</a></li>
  <li><a href="typehistoriqueevenements">typehistoriqueevenement</a></li>
  <li><a href="typepiecevehicules">typepiecevehicule</a></li>
  <li><a href="typeutilisateurs">typeutilisateur</a></li>
  <li><a href="typevehicules">typevehicule</a></li>
  <li><a href="utilisateurs">utilisateur</a></li>
  <li><a href="vehicules">vehicule</a></li>
  <li><a href="villes">Villes</a></li>
  
</ul>

<ul id="dropdown1" class="dropdown-content">
  <!-- Main catagories ( les autres sont des sous categories presentes dans les pages maitres) -->
  <li><a href="agences">Agences</a></li>
  <li><a href="clients">Clients</a></li>
  <li><a href="droits">Droits</a></li>
  <li><a href="vehicules">Vehicules</a></li>
  <li><a href="utilisateurs">Utilisateurs</a></li>
  <li><a href="villes">Villes</a></li>
  <li><a href="statuts">Statuts</a></li>
  
</ul>
<ul id="dropdown2" class="dropdown-content">
  <li><a href="/profil">Voir</a></li>
  <li><a href="/profil/parametres">Paramètres</a></li>
  <li class="divider"></li>
  <li><a href="logout" class="red-text">Déconnexion</a></li>
</ul>
<!-- Dropdown Structure -->
<div class="navbar-fixed">
  <nav role="navigation">
    <div class="nav-wrapper container">
      
      <a href="{{url('home')}}"><img style="height:90%;margin:3px" src="data:image/png;base64," alt="" class="circle responsive-img"></a>
      <ul class="right hide-on-med-and-down">
        <li><a class="dropdown-button" href="#!" data-activates="dropdown0">ALL<i class="material-icons right">arrow_drop_down</i></a></li>
        <li><a class="dropdown-button" href="#!" data-activates="dropdown1">Gestion<i class="material-icons right">arrow_drop_down</i></a></li>
        <!-- 
        <li><a href="#">Navbar Link</a></li>
        <li><a href="#">Navbar Link</a></li>
        -->
        <li><a class="dropdown-button" href="#!" data-activates="dropdown2">Mon profil<i class="material-icons right">arrow_drop_down</i></a></li>
      </ul>

      <ul id="nav-mobile" class="side-nav">
      <li><a href="/accueil"><i class="material-icons">home</i> Tableau de bord</a></li>
      <li><a href="agences"><i class="material-icons">work</i> Agences</a></li>
      <li><a href="clients"><i class="material-icons">people</i> Clients</a></li>
      <li><a href="droits"><i class="material-icons">assignment_ind</i> Droits</a></li>
      <li><a href="vehicules"><i class="material-icons">directions_car</i> Vehicules</a></li>
      <li><a href="utilisateurs"><i class="material-icons">supervisor_account</i> Utilisateurs</a></li>
      <li><a href="villes"><i class="material-icons">location_city</i> Villes</a></li>
      <li><a href="statuts"><i class="material-icons">view_list</i> Statuts</a></li>
        
      </ul>
      <a href="#" data-activates="nav-mobile" class="button-collapse"><i class="material-icons">menu</i></a>
    </div>
  </nav>
</div>